...
const assistQuestions: Question[] = [
    {
        id: 'tobacco',
...
export const SBIRTView = () => {
    ...
    const [assistScores, setAssistScores] = useState<Record<string, boolean>>({});

    const toggleScore = (id: string) => {
        setAssistScores(prev => ({ ...prev, [id]: !prev[id] }));
    };
    ...
    const getRiskCategory = (category: string, score: number) => {
        if (category === 'alcohol') {
