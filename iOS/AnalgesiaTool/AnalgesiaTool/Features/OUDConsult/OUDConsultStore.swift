...
class OUDConsultStore: ObservableObject {
    // MARK: - State
    @Published var viewMode = "consult" // consult, protocols
    ...
    // NIDA Quick Screen State
    @Published var nidaAlcohol: Bool = false
    @Published var nidaTobacco: Bool = false
    @Published var nidaRx: Bool = false
    @Published var nidaIllegal: Bool = false
    
    // Workup State
    @Published var completedWorkupItems: Set<UUID> = []
    ...
    var shouldExpandDSM: Bool {
        return nidaRx || nidaIllegal || !selectedCriteria.isEmpty
    }
    ...
    func reset() {
        ...
        nidaRx = false
        nidaIllegal = false
        completedWorkupItems.removeAll()
    }
}
