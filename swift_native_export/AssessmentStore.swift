...
class AssessmentStore: ObservableObject {
    ...
    func calculate() {
        ...
        var generalRiskScore: String = "Low"
        ...
        func addPORecs() {
            ...
            if isRenalBad {
                ...
                if !naive {
                    recs.append(DrugRecommendation(name: "Methadone PO", reason: "Safe Option.", detail: "No renal metabolites. Consult Specialist.", type: .safe))
                }
        ...
        if hemo == .unstable {
            ...
            else { addIVRecs(); addPORecs(); adj.append("Route Preference: Determine based on GI tolerance.") }
        }
            ...
            // FIX: Hydromorphone Shunt Warning
             recs = recs.map { r in
                 if r.name.contains("Hydromorphone") && r.name.contains("PO") {
                     return DrugRecommendation(name: r.name, reason: "EXTREME CAUTION", detail: "DANGER: Bioavailability increases 400% (Shunt Effect). Start 25% dose or Avoid.", type: .caution)
                 }
